  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!userRole) {
        ...
        // FIX: Added null check for user before accessing user.id
        const { data: trabajador } = await supabase.from("trabajadores").select("rol").eq("user_id", user.id).single()

        if (trabajador) {
            // FIX: The field in the `trabajadores` table seems to be `rol`, not `tipo`.
            userRole = trabajador.rol
        } else {
          // Verificar si es administrador
          // FIX: The table name is likely `farmacias` with `id_administrador` not `administradores` table.
          // This logic might need adjustment based on the actual schema. Assuming a check on the `farmacias` table.
          const { data: adminFarmacia } = await supabase.from("farmacias").select('id_farmacia').eq("id_administrador", user.id).maybeSingle()

          userRole = adminFarmacia ? "administrador" : "paciente"
        }
      ...
      switch (userRole) {
        ...
        case "Doctor":
          navigate("/doctor")
          break
        case "doctor": // Keeping this for backward compatibility
          navigate("/doctor")
          break
        case "farmaceutico":
      ...
      // FIX: Type 'err' as Error to access the message property safely.
  ...
  const handleGoogleLogin = async () => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          redirectTo: `${window.location.origin}/paciente`, // Adjusted to avoid trailing slash issues.
        },
      })
      ...
      if (err instanceof Error) {
          ...
          setError("Ocurrió un error desconocido con el inicio de sesión de Google.")
      }
    }
  }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="mt-8 text-center text-xs text-gray-500">
          ...
          <div className="flex justify-center space-x-4 mt-2">
            ...
            <Link to="/ayuda" className="hover:text-emerald-600">
              Ayuda
            </Link>
          </div>
        </div>
